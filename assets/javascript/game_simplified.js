  ...
  var game = {
    ...
    fight: function() {
        ...
        $userprops = '<h6><strong>' + $user.name + '</strong></h6>' +
                     '<p>Health: ' + $user.health_points + '</p>';
        $floatingPoints = '<p class="floating-points animated fadeOutUp">' + $opponent.attack_power + '</p>';

        // append player properties
        game.userPropsContainer.html($userprops);
        game.userPropsContainer.prepend($floatingPoints);
        ...
        $opponentprops = '<h6><strong>' + $opponent.name + '</strong></h6>' +
                         '<p>Health: ' + $opponent.health_points + '</p>';
        $floatingPoints = '<p class="floating-points animated fadeOutUp">' + $user.attack_power + '</p>';

        // append player properties
        game.opponentPropsContainer.html($opponentprops);
        game.opponentPropsContainer.prepend($floatingPoints);
